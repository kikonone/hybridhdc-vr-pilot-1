...
directory=base/'results/checkpoints/linear_svm'
files=[directory/f'linear_svm_fold_{fold}_predictions.csv' for fold in [1,2,3,4,5]]
if not all(path.is_file() for path in files): raise RuntimeError('Incomplete Linear SVM checkpoints')
...
oof.to_csv(base/'results/predictions/linear_svm_oof.csv',index=False)
